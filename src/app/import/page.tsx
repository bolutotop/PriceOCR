...
import { Loader2, Save, UploadCloud, ImageIcon, X, AlertTriangle, Search, CheckCircle2, RefreshCw } from 'lucide-react';
  ...
  useEffect(() => {
    ...
    if (status === 'processing') {
      ...
      interval = setInterval(() => {
        setProgress((prev) => {
          if (prev < 30) { setProgressText('正在预处理图像...'); return prev + 10; }
          if (prev < 70) { setProgressText('OCR 引擎识别中...'); return prev + 5; }
          if (prev < 90) { setProgressText('智能分栏与匹配...'); return prev + 2; }
          return prev;
        });
      }, 200);
    ...
    try {
      ...
      if (res.success) {
        ...
        if (confirm('保存成功！是否查看历史记录？')) {
           ...
           setFile(null); setImagePreview(null); setItems([]); setStatus('idle');
        }
      } else {
    ...
    <div className="min-h-screen bg-slate-50 pb-20">
      ...
      <div className="bg-white border-b sticky top-0 z-30 px-4 md:px-6 py-3 flex justify-between items-center shadow-sm">
        <div>
          ...
          <p className="text-xs text-slate-500 hidden sm:block">上传图片 &rarr; 智能识别 &rarr; 确认保存</p>
        </div>
            ...
            <CardContent className="p-4">
              ...
              {!file ? (
                ...
                <div 
                  ...
                  className="border-2 border-dashed border-slate-300 rounded-xl h-40 lg:h-56 flex flex-col items-center justify-center text-slate-500 bg-slate-50/50 hover:bg-slate-100 hover:border-blue-400 transition cursor-pointer gap-3 group"
                >
                  ...
                  <div className="text-center">
                    <p className="text-sm font-medium text-slate-700">点击上传报价单</p>
                  </div>
                </div>
              ) : (
                // 已选文件 (优化：限制最大高度 max-h-[300px]，防止图太大)
                <div className="space-y-4">
                  <div className="relative rounded-xl overflow-hidden bg-slate-100 border border-slate-200 group flex justify-center items-center">
                    ...
                    <div className="absolute inset-0 bg-black/5 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                       <Button variant="secondary" size="sm" className="bg-white/90 shadow-sm backdrop-blur-sm" onClick={() => fileInputRef.current?.click()}>
                         <RefreshCw className="w-3.5 h-3.5 mr-1.5" /> 更换图片
                       </Button>
                    </div>
                  </div>
                  ...
                  >
                    {status === 'processing' ? <Loader2 className="animate-spin mr-2"/> : <Search className="w-5 h-5 mr-2"/>}
                    {status === 'success' ? '识别成功' : status === 'processing' ? '正在分析...' : '开始智能识别'}
                  </Button>
                </div>
              )}
            </CardContent>
        ...
        <div className="lg:col-span-8 flex flex-col h-[calc(100vh-140px)]">
          ...
          <Card className="flex-1 flex flex-col border-slate-200 shadow-sm overflow-hidden bg-white">
            ...
            <div className="flex-1 overflow-auto relative scrollbar-hide bg-white">
              ...
              {items.length > 0 && (
                <Table>
                  {/* 表头：Sticky 吸附顶部，背景纯白防止透视 */}
                  <TableHeader className="sticky top-0 z-20 bg-white shadow-sm border-b border-slate-100">
                    <TableRow className="text-xs hover:bg-white text-slate-500 font-medium">
                      <TableHead className="pl-3 md:pl-6 w-auto">参考截图</TableHead>
                      <TableHead className="pl-2 md:pl-4">品名 (点击编辑)</TableHead>
                      <TableHead className="w-[90px] md:w-[110px] text-right pr-3 md:pr-6">价格</TableHead>
                      <TableHead className="w-[40px] md:w-[50px]"></TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {filteredItems.map((item, idx) => (
                      // 优化：py-3 增加纵向高度，让每行更舒展
                      <TableRow key={idx} className="group text-sm hover:bg-blue-50/30 transition-colors border-slate-100">
                        
                        {/* 截图：尺寸缩小为 h-10 (Mobile) / h-14 (PC) */}
                        <TableCell className="py-3 pl-3 md:pl-6 align-middle">
                          {item.cropDataUri ? (
                        ...
                        {/* 价格 */}
                        <TableCell className="py-3 text-right pr-3 md:pr-6 align-middle">
                          <input 
                        ...
                        {/* 删除 */}
                        <TableCell className="py-3 align-middle text-right px-0 md:pr-2">
                          <Button 
                            variant="ghost" 
                            size="icon" 
